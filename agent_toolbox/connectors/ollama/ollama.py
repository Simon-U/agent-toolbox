...
def parse_response(message: BaseMessage) -> str:
    ...
    if isinstance(message, AIMessage):
        ...
        if tool_calls:
            ...
            parsed_chat_result = json.loads(content)
        ...
        if len(tool_calls) > 0:
            ...
            raise ValueError(f"`tool_calls` missing from AIMessage: {message}")
...
class Ollama(ChatOllama):
    ...
    def __init__(
        self, base_url: str = "http://103.133.98.204:11434", **kwargs: Any
    ) -> None:
    ...
    def extract_curly_braces_content(self, text):
        # Use regex to find the content within the curly braces
        match = re.search(r"\{.*\}", text, re.DOTALL)
        if match:
            if "response" in text:
                text = text.split('"response":')[-1].split("}")[0].strip().strip('"')
            else:
                return match.group(0)

        text = text.replace('"', "<double_quotes>").replace("\n", "<new_line>")
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        try:
            parsed_text = self.extract_curly_braces_content(chat_generation_content)
            parsed_chat_result = json.loads(parsed_text)
        ...
        if (
            ...
            final_response = (
                response.replace("<double_quotes>", '"')
                .replace("tool: __conversational_response<new_line>response:", "")
                .replace("<new_line>", "\n")
                .strip()
            )
